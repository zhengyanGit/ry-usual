	...
	M.ui.define('waiting', function () {
		...
		Waiting.prototype = {
			...
			resolve: function (icon, that, args) {
				if (this.ops.isLock) {
					this.ops.lockWrap = M(M.creatlabel(), {
						'class': 'g-master-container g-confirm-master mt-loading-wraper',
					...
					M(document.body).append(this.ops.lockWrap)
				}
			...
			remove: function (that, args, type, top) {
				if (M.isObject(that)) {
					...
					if (that.ops) {
						that.ops.isRemoved = true;
						if (that.ops.isLock && that.ops.lockWrap) {
							that.ops.lockWrap.remove();
						}
					}
				} else {
					M('.mt-waiting-master').not('.alone').remove();
					M('.mt-loading-wraper').remove();
				};
			},
