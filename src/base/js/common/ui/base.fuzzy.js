...
//				}[index]);
            ...
            resolve:function(data,args){
                for(var i=0,str='';i<data.length;i++){
                    str+=M.template.resolve(this.ops.tip?this.renderItemTip():this.renderItem(),{name:data[i].customerName,value:data[i].comId,tax:data[i].taxNum,tip:data[i].s,className:i==0?'active':''});
                };
            ...
            get:function(fn){
                if(this.ops.data){
                    ...
                    M.ui.ajax.init({
                        url:this.ops.url,
                        data:M.extend({}, this.ops.param, { "customerName": _value }),
                        isload:false,
                        type:'post',
                        dataType: 'json',
                        beforeSend:function(){
