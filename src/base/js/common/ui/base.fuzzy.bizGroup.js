 ...
 */
;(function(M,window){
    M.ui.define('fuzzyBizGroup',function(){
...
//				}[index]);
            ...
            select:function(e){
                ...
                that.email=this.getAttribute('email');
                that.mobile=this.getAttribute('mobile');
                that.nickName=this.getAttribute('nickName');
                that.change();
                that.hide();
            ...
            resolve:function(data,args){
                for(var i=0,str='';i<data.length;i++){
                    str+=M.template.resolve(this.ops.tip?this.renderItemTip():this.renderItem(),
                        {
            ...
            get:function(fn){
                if(this.ops.data){
                    ...
                    M.ui.ajax.init({
                        url:this.ops.url,
                        data:M.extend({}, this.ops.param, { "empNameLk": _value }),
                        isload:false,
                        type:'post',
                        dataType: 'json',
                        beforeSend:function(){
                        ...
                        success:function(res){
                            if (res.success) {
                                ...
                                res.data.forEach(function (item) {
                                    if (item.empName.indexOf(text) > -1) {
                                        var temp;
                                        if (item.empName != undefined) {
                                            temp = {
            ...
            renderItem:function(){
                return '<li class="{{className}}" nickName="{{nickName}}" mobile="{{mobile}}" email="{{email}}">{{empName}}</li>';
            },
            renderItemTip:function(){
                return '<li class="{{className}}" uid="{{value}}">{{empName}}<i class="tip">{{tip}}</i></li>';
            },
