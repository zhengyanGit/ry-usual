 ...
 */
; (function (M, window) {
	M.ui.define('fuzzyCustomerName', function () {
		...
		Fuzzy.prototype = {
			...
			select: function (e) {
				...
				that.orgName = this.getAttribute('orgName');
				that.orgId = this.getAttribute('orgId');
				that.change();
				that.hide();
			...
			change: function () {
				...
				this.targetElement.attr('orgName', this.orgName)
				this.targetElement.attr('orgId', this.orgId)
				this.valueElement.val(this.value);
			},
			resolve: function (data, args) {
				for (var i = 0, str = ''; i < data.length; i++) {
					str += M.template.resolve(this.ops.tip ? this.renderItemTip() : this.renderItem(), { name: data[i].customerName, orgName: data[i].orgName, orgId: data[i].orgId, tip: data[i].s, className: i == 0 ? 'active' : '' });
				};
			...
			get: function (fn) {
				if (this.ops.data) {
					...
					var _value = this.targetElement[0].value;
					if (_value.length > 0 && _value.lastIndexOf("（") != -1) {
						_value = _value.substr(0, _value.lastIndexOf("（"));
					}
					M.ui.ajax.init({
						url: this.ops.url,
						data: M.extend({}, this.ops.param, { "customerNameLk": _value }),
						isload: false,
						type: 'get',
						dataType: 'json',
						beforeSend: function () {
						...
						success: function (res) {
							if (res.success) {
								...
								res.data.forEach(function (item) {
									if (item.customerName.indexOf(text) > -1) {
										var temp;
										if (item.orgName != undefined) {
											temp = {
												"comId": item.customerId,
												"s": item.customerName,
												"n": item.customerName,
												"customerName": item.customerName + '（' + item.orgName + '）',
												"orgId": item.orgId,
												"orgName": item.orgName,
												"taxNum": item.customerName
											};
										} else {
											temp = {
												"comId": item.customerId,
			...
			renderItem: function () {
				return '<li class="{{className}}" orgName="{{orgName}}" orgId="{{orgId}}">{{name}}</li>';
			},
			renderItemTip: function () {
				return '<li class="{{className}}" orgName="{{orgName}}">{{name}}<i class="tip">{{tip}}</i></li>';
			},
